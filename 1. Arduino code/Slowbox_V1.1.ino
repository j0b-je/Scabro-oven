...
bool FileNameCheck = true; // This was false
...
void setup()
{
  ...
  ThermoCorrection1 = ThermoVal1 * 0.25;
  ThermoCorrection2 = ThermoVal2 * 0.25;
  ThermoCorrection3 = ThermoVal3 * 0.25;
  ThermoCorrection4 = ThermoVal4 * 0.25;

  DebugSerial.println("OffSet form EEPROM for Thermocouple 4: ");
  DebugSerial.println(ThermoVal1);
  DebugSerial.println(ThermoCorrection1);
  ...
  sendCmd("ArduinoV.txt = \"Beta V4.2 SlowBox\"");
...
void loop()
{
  ...
  DebugSerial.print("SpeedSet LIVE2: ");
  DebugSerial.println(SpeedSet);

  DebugSerial.print("TIMR LIVE: ");
  DebugSerial.println(TIMR);

  DebugSerial.print("TEMPR LIVE: ");
  DebugSerial.println(TEMPR);

  DebugSerial.print("FileNameCheck LIVE: ");
  ...
  if (NextionSerial.available() > 0) {
    ...
    if (c == '>') {
      DebugSerial.println("Recieved Nextion Data request");
      SendData = true;
      readString = ""; //clears variable for new input
    }
  ...
  while (ThermoProces == true && Dataprocessing == false) {
    ...
    if (Thermocounter >= 200) {
      TempOffSet1 = int(thermocouple.readCelsius() + ThermoCorrection);
      TempOffSet2 = int(thermocouple1.readCelsius() + ThermoCorrection2);
      TempOffSet3 = int(thermocouple2.readCelsius() + ThermoCorrection3);
      TempOffSet4 = int(thermocouple3.readCelsius() + ThermoCorrection4);
    ...
    THC1 = ThermoVal1 * 0.25;
    THC2 = ThermoVal2 * 0.25;
    THC3 = ThermoVal3 * 0.25;
    THC4 = ThermoVal4 * 0.25;
  ...
  if (Dataprocessing == false && ThermoProces == false) {
    ...
    float RawTemperature = (thermocouple.readCelsius());
    ...
    DebugSerial.println(thermocouple.readCelsius() + ThermoCorrection1);

    DebugSerial.print("Raw Value Th 2: ");
    DebugSerial.println(thermocouple1.readCelsius() + ThermoCorrection2);

    DebugSerial.print("Raw Value Th 3: ");
    DebugSerial.println(thermocouple2.readCelsius() + ThermoCorrection3);

    DebugSerial.print("Raw Value Th 4: ");
    DebugSerial.println(thermocouple3.readCelsius() + ThermoCorrection4);


    if (TIMR == true && TEMPR == true && FileNameCheck == true) {

      DebugSerial.println("Starting First stage of the preprogramma");
      ...
      if (thermocouple.readCelsius() + ThermoCorrection1 >= 0 && thermocouple1.readCelsius() + ThermoCorrection2 >= 0 && thermocouple2.readCelsius() + ThermoCorrection2 >= 0)
      {
        ThermoCheck = 1;
      }

      if (thermocouple.readCelsius() + ThermoCorrection <= 0 || thermocouple1.readCelsius() + ThermoCorrection1 <= 0 || thermocouple2.readCelsius() + ThermoCorrection2 <= 0)// can be subsituded for a else statment
      {
        ThermoCheck = 0;
    ...
    if (RunningState == 1 && ThermoCheck == 1 && SpeedSet >= 0) {
      ...
      if (currentMillis - WriteSDDelta >= 5000 && check == true) {
        ...
        myFile.print(thermocouple.readCelsius() + ThermoCorrection1);
        myFile.print(":");
        myFile.print(thermocouple1.readCelsius() + ThermoCorrection2);
        myFile.print(":");
        myFile.print(thermocouple2.readCelsius() + ThermoCorrection3);
      ...
      */
      ...
      if ((thermocouple.readCelsius() + ThermoCorrection) >= SetTemp + 2 || ((thermocouple1.readCelsius() + ThermoCorrection1) + (thermocouple2.readCelsius() + ThermoCorrection2) / 2) >= SetTemp - 2) Setpoint = SetTemp;
      ...
      else   Setpoint = SetTemp + 2;

      if ((((thermocouple1.readCelsius() + ThermoCorrection1) + (thermocouple2.readCelsius() + ThermoCorrection2)) / 2) >= SetTemp - 3) {
        if (TTS == false)
        {
          Setpoint = SetTemp;
      ...
      if (SpeedSet == 1) {
        RampTime = 1;
        if (Setpoint >= 100) myPID.setOutputRange(0, 120);
        if (Setpoint >= 90) myPID.setOutputRange(0, 90);
        if (Setpoint >= 80) myPID.setOutputRange(0, 70);
        if (Setpoint >= 60) myPID.setOutputRange(0, 60);
        else myPID.setOutputRange(0, 50);
      }
      if (SpeedSet == 2) {
        RampTime = 2;
        if (Setpoint >= 100) myPID.setOutputRange(0, 120);
        if (Setpoint >= 90) myPID.setOutputRange(0, 100);
        if (Setpoint >= 70) myPID.setOutputRange(0, 80);
        else myPID.setOutputRange(0, 60);
      }
      if (SpeedSet == 3) {
        RampTime = 4;
        if (Setpoint >= 115) myPID.setOutputRange(0, 120);
        if (Setpoint >= 105) myPID.setOutputRange(0, 115);
        if (Setpoint >= 95) myPID.setOutputRange(0, 110);
        if (Setpoint >= 75) myPID.setOutputRange(0, 100);
